...
- Styling rules from `## Frontend > Styling rules` are non-negotiable: Tailwind everywhere, project tokens (`surface-*` / `accent-*` / `text-*` / `border-*` and the `surface-card` / `btn-primary` / `btn-secondary` / `input-field` component classes), `lucide-react` icons, MUI only for `@mui/x-charts`, no `sx={}` outside chart styling, no direct `@emotion/*` imports, no `@mui/icons-material`. Modals reuse `common/Modal.tsx` / `common/DeleteConfirmDialog.tsx`. Forms reuse `useFormState`. Uploads reuse `useFileUpload`.
